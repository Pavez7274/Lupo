...
exports.default = {
    ...
    run: (d) => {
        let cmds = d.lappy.cmds.default
            .map(({ names }) => names[0])
            .sort((a, b) => a.localeCompare(b)), array = [];
        ...
        ;
        let cmd = d.lappy.cmds.default.find((cmd) => cmd.names.includes(d.args.get(0)));
        if (!cmd)
        ...
        if (cmd.fields) {
            embeds[0].fields?.push({
                name: 'Fields',
                value: cmd.fields.map((field) => `${field.name.concat(field.req ? '' : '?')} -> ${field.type}`).join('\n').toTitleCase().toCodeBlock()
            });
        }
        ;
        if (cmd.dev) {
