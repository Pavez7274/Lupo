  ...
  let start = Date.now(),
   ...
   asynchronous = d.args.endIsTrue('(asynchronous|async)');
	...
	if (evaled && typeof_1 === 'date') {
		...
		result = `<t:${time}>\n\`${evaled}\``;
	} else result = result.cropAt(1000).toCodeBlock('js');
	// makes the embed
	if (d.args.endIsFalse('embed')) {
		...
		if (typeof_0 == 'string' && evaled?.startsWith('https://') && axios.get(result).then(({ headers }) => headers?.['content-type']?.includes('image')).catch(() => false))
